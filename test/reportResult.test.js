...
describe("reportResult", function () {
  ...
  it("should pay the single winner the full prize pool minus oracle fee (2 players)", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, alice, winnerA } = await lockedBetFixture();
    ...
    const expectedPayout = prizePool - ORACLE_FEE;
  ...
  it("should pay the single winner the full prize minus oracle fee (3 players)", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, alice, winnerA } = await lockedBet3PlayersFixture();
    const aliceBefore = await usdt.balanceOf(alice.address);
    await bgamble.connect(oracle1).reportResult(1, [winnerA]);
    await bgamble.connect(oracle2).reportResult(1, [winnerA]);
    await bgamble.connect(oracle3).reportResult(1, [winnerA]);
  ...
  it("should pay nothing to the loser", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, bob, winnerA } = await lockedBetFixture();
  ...
  it("should split payout equally between 2 winners (3 players)", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, alice, bob, winnerA, winnerB } =
  ...
  it("should give dust (remainder) to the last winner", async function () {
    ...
    const wB = 2;
    const wC = 3;
    await bgamble.connect(alice).create(1, elevenPol, 3, wA);
    ...
    const prizePool = elevenPol * USDT_UNIT * 3n;
    const payout = prizePool - ORACLE_FEE;
    const share = payout / 3n;
    const remainder = payout % 3n;

    expect(await usdt.balanceOf(alice.address) - aliceBefore).to.equal(share);
    expect(await usdt.balanceOf(bob.address) - bobBefore).to.equal(share);
    expect(await usdt.balanceOf(carol.address) - carolBefore).to.equal(share + remainder);
  });
  ...
  it("should split payout among all participants when everyone predicted the winner", async function () {
    ...
    expect(await usdt.balanceOf(alice.address) - aliceBefore).to.equal(share);
    expect(await usdt.balanceOf(bob.address) - bobBefore).to.equal(share + payout % 2n);
  });
  ...
  it("should refund all (minus fee) when no participant predicted the winner", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, alice, bob } = await lockedBetFixture();
  ...
  it("should refund all (no fee) when 4 oracles report different results", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, oracle4, alice, bob } = await lockedBetFixture();
  ...
  it("should pay oracle fee to oracle at index betId % 4 (betId=1 → oracle[1])", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3 } = await lockedBetFixture();
    // betId=1, fee recipient = oracles[1 % 4] = oracle2
    const oracle2Before = await usdt.balanceOf(oracle2.address);
    const winnerA = 1;
    await bgamble.connect(oracle1).reportResult(1, [winnerA]);
    await bgamble.connect(oracle2).reportResult(1, [winnerA]);
    await bgamble.connect(oracle3).reportResult(1, [winnerA]);
    const oracle2After = await usdt.balanceOf(oracle2.address);
  ...
  it("should pay oracle fee to oracle[0] when betId % 4 == 0", async function () {
    ...
    await bgamble.connect(alice).create(2, TEN_USDT, 2, w);
    await bgamble.connect(alice).create(3, TEN_USDT, 2, w);

    // Bet 4: lock and resolve
    await bgamble.connect(alice).create(4, TEN_USDT, 2, w);
    ...
    const oracle1Before = await usdt.balanceOf(oracle1.address);
    ...
    const oracle1After = await usdt.balanceOf(oracle1.address);
  ...
  it("should leave the contract with 0 balance after resolution (single winner)", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, winnerA } = await lockedBetFixture();
    await bgamble.connect(oracle1).reportResult(1, [winnerA]);
    await bgamble.connect(oracle2).reportResult(1, [winnerA]);
    await bgamble.connect(oracle3).reportResult(1, [winnerA]);
  ...
  it("should leave the contract with 0 balance after resolution (multiple winners)", async function () {
    const { bgamble, usdt, oracle1, oracle2, oracle3, winnerA, winnerB } = await lockedBet3PlayersFixture();
  ...
  it("should revert if bet is Confirming", async function () {
    const { bgamble, oracle1 } = await confirmingBetFixture();
    const w = 1;
    await expect(bgamble.connect(oracle1).reportResult(1, [w])).to.be.revertedWithCustomError(bgamble, "BetNotLocked");
  });
  ...
  it("should revert if bet was Cancelled", async function () {
    const { bgamble, oracle1, alice, bob } = await lockedBetFixture();
