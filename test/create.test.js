import { describe, it } from "node:test";
import { expect } from "chai";
import { deployFixture, TEN_USDT, MIN_BET_AMOUNT_USDT, MAX_BET_AMOUNT_USDT, USDT_UNIT } from "./helpers.js";

describe("create", function () {
  // ── Positive tests ──────────────────────────────────────────────

  it("should create a bet and return betId 1", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    const betId = await bgamble.connect(alice).create.staticCall(1, TEN_USDT, 2, winner);
    expect(betId).to.equal(1);
  });

  it("should increment nextBetId after creation", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 2, winner);
    expect(await bgamble.nextBetId()).to.equal(2);
  });

  it("should increment nextBetId for each new bet", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 2, winner);
    await bgamble.connect(alice).create(2, TEN_USDT, 2, winner);
    await bgamble.connect(alice).create(3, TEN_USDT, 2, winner);
    expect(await bgamble.nextBetId()).to.equal(4);
  });

  it("should set bet state to Open", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 2, winner);
    const bet = await bgamble.bets(1);
    expect(bet.state).to.equal(0); // BetState.Open
  });

  it("should store the correct bet amount", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 3, winner);
    const bet = await bgamble.bets(1);
    expect(bet.amount).to.equal(TEN_USDT);
  });

  it("should store the correct slot count", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 5, winner);
    const bet = await bgamble.bets(1);
    expect(bet.slotCount).to.equal(5);
  });

  it("should auto-join the creator as the first participant", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 2, winner);

    const participants = await bgamble.getParticipants(1);
    expect(participants.length).to.equal(1);
    expect(participants[0].addr).to.equal(alice.address);
    expect(participants[0].predictedWinner).to.equal(winner);
  });

  it("should transfer the stake from the creator to the contract", async function () {
    const { bgamble, usdt, alice } = await deployFixture();
    const winner = 1;
    const balanceBefore = await usdt.balanceOf(alice.address);
    await bgamble.connect(alice).create(1, TEN_USDT, 2, winner);
    const balanceAfter = await usdt.balanceOf(alice.address);
    // USDT transfer is exact (no gas deducted from token balance)
    expect(balanceBefore - balanceAfter).to.equal(TEN_USDT * USDT_UNIT);
  });

  it("should emit BetCreated event", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    const tableId = 822871361;
    await expect(bgamble.connect(alice).create(tableId, TEN_USDT, 2, winner))
      .to.emit(bgamble, "BetCreated")
      .withArgs(1, tableId, () => true, alice.address, TEN_USDT, 2, winner);
  });

  it("should emit BetJoined event for the creator", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, TEN_USDT, 2, winner))
      .to.emit(bgamble, "BetJoined")
      .withArgs(1, () => true, alice.address, winner);
  });

  it("should work with minimum slot count of 2", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 2, winner);
  });

  it("should work with maximum slot count of 10", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, TEN_USDT, 10, winner);
  });

  it("should work with minimum amount of 5 USDT", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, MIN_BET_AMOUNT_USDT, 2, winner);
  });

  it("should work with maximum amount of 250 USDT", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await bgamble.connect(alice).create(1, MAX_BET_AMOUNT_USDT, 2, winner);
  });

  it("should work with a large amount within the max bound", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    const largeAmount = 200n;
    await bgamble.connect(alice).create(1, largeAmount, 2, winner);
  });

  // ── Negative tests ──────────────────────────────────────────────

  it("should revert with slotCount of 0", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, TEN_USDT, 0, winner)).to.be.revertedWithCustomError(bgamble, "SlotCountTooLow");
  });

  it("should revert with slotCount of 1", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, TEN_USDT, 1, winner)).to.be.revertedWithCustomError(bgamble, "SlotCountTooLow");
  });

  it("should revert with slotCount of 11", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, TEN_USDT, 11, winner)).to.be.revertedWithCustomError(bgamble, "SlotCountTooHigh");
  });

  it("should revert with slotCount of 255", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, TEN_USDT, 255, winner)).to.be.revertedWithCustomError(bgamble, "SlotCountTooHigh");
  });

  it("should revert with amount of 0", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, 0, 2, winner)).to.be.revertedWithCustomError(bgamble, "BetAmountTooLow");
  });

  it("should revert with amount less than 5 USDT (e.g. 4)", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    await expect(bgamble.connect(alice).create(1, 4, 2, winner)).to.be.revertedWithCustomError(bgamble, "BetAmountTooLow");
  });

  it("should revert with amount exceeding 250 USDT", async function () {
    const { bgamble, alice } = await deployFixture();
    const winner = 1;
    const tooMuch = MAX_BET_AMOUNT_USDT + 1n;
    await expect(bgamble.connect(alice).create(1, tooMuch, 2, winner)).to.be.revertedWithCustomError(bgamble, "BetAmountTooHigh");
  });

  it("should revert if caller has not approved enough USDT", async function () {
    const { bgamble, usdt, alice } = await deployFixture();
    const winner = 1;
    // Revoke approval
    await usdt.connect(alice).approve(await bgamble.getAddress(), 0);
    await expect(bgamble.connect(alice).create(1, TEN_USDT, 2, winner))
      .to.be.revertedWithCustomError(usdt, "ERC20InsufficientAllowance");
  });
});
