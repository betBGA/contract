...
import { deployFixture, createOpenBetFixture, confirmingBetFixture, lockedBetFixture, lockedBet3PlayersFixture, TEN_USDC, ONE_DAY } from "./helpers.js";
...
describe("view functions", function () {
  ...
  describe("getResultVotes", function () {
    ...
    it("should track separate vote counts for different hashes", async function () {
      ...
      const hashB = ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode(["uint64[]"], [[winnerB]])
      );
  ...
  describe("getResolvedWinnerIds", function () {
    ...
    it("should return correct multiple winners after resolution", async function () {
      const { bgamble, oracle1, oracle2, oracle3, winnerA, winnerB, winnerC } = await lockedBet3PlayersFixture();
  ...
  describe("getBetSummary", function () {
    ...
    it("should return correct data for an Open bet", async function () {
      ...
      expect(s.amount).to.equal(TEN_USDC);
  ...
  describe("getBetsByState", function () {
    it("should return empty array when no bets exist", async function () {
      const { bgamble } = await deployFixture();
      const results = await bgamble.getBetsByState(0, 0, 20, false); // Open, desc
    ...
    it("should return empty array for a state with no matching bets", async function () {
      ...
      const results = await bgamble.getBetsByState(2, 0, 20, false); // Locked
      expect(results.length).to.equal(0);
    });
    ...
    it("should return Open bets (descending)", async function () {
      const { bgamble, alice, bob, ethers } = await deployFixture();
      const w = 1;
      // Create 3 open bets (slotCount=3 so they stay Open after create)
      await bgamble.connect(alice).create(1, TEN_USDC, 3, w);
      await bgamble.connect(alice).create(2, TEN_USDC, 3, w);
      await bgamble.connect(alice).create(3, TEN_USDC, 3, w);
    ...
    it("should return Open bets (ascending)", async function () {
      ...
      const results = await bgamble.getBetsByState(0, 0, 20, true); // Open, asc
      expect(results.length).to.equal(3);
    ...
    it("should filter by state when bets have mixed states", async function () {
      ...
      // Bet 1: Open (3-player, only 1 joined)
      await bgamble.connect(alice).create(1, TEN_USDC, 3, wA);
      // Bet 2: Confirming (2-player, both joined, none confirmed)
      await bgamble.connect(alice).create(2, TEN_USDC, 2, wA);
      await bgamble.connect(bob).join(2, wB);
      // Bet 3: Open (3-player, only 1 joined)
      await bgamble.connect(alice).create(3, TEN_USDC, 3, wA);

      const openBets = await bgamble.getBetsByState(0, 0, 20, false); // Open
      expect(openBets.length).to.equal(2);
      expect(openBets[0].betId).to.equal(3);
      expect(openBets[1].betId).to.equal(1);

      const confirmingBets = await bgamble.getBetsByState(1, 0, 20, false); // Confirming
    ...
    it("should respect the limit parameter", async function () {
      ...
      const results = await bgamble.getBetsByState(0, 0, 2, false); // limit=2, desc
    ...
    it("should paginate with cursor (descending)", async function () {
      ...
      await bgamble.connect(alice).create(2, TEN_USDC, 3, w);
      await bgamble.connect(alice).create(3, TEN_USDC, 3, w);
      await bgamble.connect(alice).create(4, TEN_USDC, 3, w);
      await bgamble.connect(alice).create(5, TEN_USDC, 3, w);
      ...
      // Page 2: use last betId from page 1 as cursor
    ...
    it("should skip non-matching bets during pagination", async function () {
      ...
      // Bet 1: Open, Bet 2: Confirming, Bet 3: Open, Bet 4: Open
      await bgamble.connect(alice).create(1, TEN_USDC, 3, wA);   // Open
      await bgamble.connect(alice).create(2, TEN_USDC, 2, wA);   // will be Confirming
      await bgamble.connect(bob).join(2, wB);
      await bgamble.connect(alice).create(3, TEN_USDC, 3, wA);   // Open
      await bgamble.connect(alice).create(4, TEN_USDC, 3, wA);   // Open

      // Page 1: limit 2, desc → should get bet 4, 3 (skipping bet 2)
      const page1 = await bgamble.getBetsByState(0, 0, 2, false);
      expect(page1.length).to.equal(2);
      expect(page1[0].betId).to.equal(4);
      expect(page1[1].betId).to.equal(3);

      // Page 2: should get bet 1 (skipping bet 2)
      const page2 = await bgamble.getBetsByState(0, page1[1].betId, 2, false);
      expect(page2.length).to.equal(1);
    ...
    it("should return Locked bets for the oracle", async function () {
      ...
      // Bet 1: Locked
      await bgamble.connect(alice).create(1, TEN_USDC, 2, wA);
      await bgamble.connect(bob).join(1, wB);
      await bgamble.connect(alice).confirm(1);
      await bgamble.connect(bob).confirm(1);

      // Bet 2: Open
      await bgamble.connect(alice).create(2, TEN_USDC, 3, wA);

      const locked = await bgamble.getBetsByState(2, 0, 20, false); // Locked
      expect(locked.length).to.equal(1);
      expect(locked[0].betId).to.equal(1);
      expect(locked[0].state).to.equal(2); // Locked
      expect(locked[0].participants.length).to.equal(2);
    });

    it("should return Resolved bets", async function () {
      const { bgamble, oracle1, oracle2, oracle3, alice, bob, ethers } = await deployFixture();
      const wA = 1;
      const wB = 2;

      // Create and lock bet 1
      await bgamble.connect(alice).create(1, TEN_USDC, 2, wA);
      await bgamble.connect(bob).join(1, wB);
      await bgamble.connect(alice).confirm(1);
      await bgamble.connect(bob).confirm(1);
      ...
      const resolved = await bgamble.getBetsByState(3, 0, 20, false); // Resolved
      expect(resolved.length).to.equal(1);
      expect(resolved[0].betId).to.equal(1);
      expect(resolved[0].state).to.equal(3); // Resolved
    });

    it("should return NoConsensus bets", async function () {
      const { bgamble, oracle1, oracle2, oracle3, oracle4, alice, bob, ethers } = await deployFixture();
      const wA = 1;
      const wB = 2;
      ...
      // 4 oracles report different results → no consensus
      await bgamble.connect(oracle1).reportResult(1, [101]);
      await bgamble.connect(oracle2).reportResult(1, [102]);
      await bgamble.connect(oracle3).reportResult(1, [103]);
      await bgamble.connect(oracle4).reportResult(1, [104]);

      const noConsensus = await bgamble.getBetsByState(4, 0, 20, false); // NoConsensus
      expect(noConsensus.length).to.equal(1);
      expect(noConsensus[0].betId).to.equal(1);
      expect(noConsensus[0].state).to.equal(4); // NoConsensus
    });
    ...
    it("should return Cancelled bets", async function () {
      ...
      const cancelled = await bgamble.getBetsByState(5, 0, 20, false); // Cancelled
      expect(cancelled.length).to.equal(1);
      expect(cancelled[0].betId).to.equal(1);
      expect(cancelled[0].state).to.equal(5); // Cancelled
    });

    it("should return Refunded bets", async function () {
      const { bgamble, alice, bob, ethers, networkHelpers } = await deployFixture();
      const wA = 1;
      const wB = 2;
    ...
    it("should include participants in each returned summary", async function () {
      ...
      const results = await bgamble.getBetsByState(1, 0, 20, false); // Confirming
